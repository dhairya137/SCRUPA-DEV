    ...
    if file and allowed_file(file.filename):
        ...
        try:
            ...
            if filename[-3:] == "zip":
                ...
                create_new = not data_loader.table_exists(table_name, dataset_id)
                if create_new:
                    data_loader.process_excel(path, dataset_id, table_name, table_description=table_desc, type_deduction=type_deduction)
                else:
                    data_loader.process_excel(path, dataset_id, table_name, table_description=table_desc, append=True, type_deduction=type_deduction)
...
@data_service.route('/datasets/<int:dataset_id>/tables/<string:table_name>/edit_row', methods=['POST'])
def get_table_by_idd(dataset_id, table_name,idd):
    print('....'+idd)
    ...
    try:
        print(idd)
        table = data_loader.get_table_by_id(dataset_id, table_name,idd)
        print(table)
        ...
        return render_template('data_service/table-viewb.html', table=table,
                               time_date_transformations=time_date_transformations,
                               statistics=statistics, raw_table_exists=raw_table_exists, backups=backups)
